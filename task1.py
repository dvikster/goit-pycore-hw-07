...
def add_contact(args, book: AddressBook):
    #print(f"add_contact received args: {args}")  # Друк для перевірки вмісту args
    ...
    #print(f"Parsed name: {name}, phone: {phone}")  # Перевірка значень name і phone

    record = book.find(name)

    if record == "Contact not found.":  # Якщо контакт не знайдений, додаємо його
        record = Record(name)
...
def add_birthday(args, book: AddressBook):
    ...
    # Перевіряємо, чи дата народження вже існує
    if record.birthday:
...
def main():
    ...
    while True:
        ...
        #print(f"Command: {command}, Args: {args}")  # Друк для перевірки розбиття на команду та аргументи
        ...
        if command in ["close", "exit"]:
            ...
            print(add_contact(*args, book))  # Передаємо args як є
        elif command == "change":
